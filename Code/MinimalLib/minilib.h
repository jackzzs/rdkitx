...
class JSMolBase {
 public:
  ...
  virtual ~JSMolBase(){};
  virtual const RDKix::RWMol &get() const = 0;
  virtual RDKix::RWMol &get() = 0;
...
class JSMol : public JSMolBase {
 public:
  JSMol() : d_mol(new RDKix::RWMol()) {}
  JSMol(RDKix::RWMol *mol) : d_mol(mol) { checkNotNull(); }
  JSMol(const JSMol &other) { d_mol.reset(new RDKix::RWMol(other.get())); }
  JSMol &operator=(const JSMol &other) {
    d_mol.reset(new RDKix::RWMol(other.get()));
    return *this;
  }
  const RDKix::RWMol &get() const {
    checkNotNull();
    return *d_mol.get();
  }
  RDKix::RWMol &get() {
...
class JSMolShared : public JSMolBase {
 public:
  JSMolShared() = delete;
  JSMolShared(const RDKix::ROMOL_SPTR &mol) : d_mol(mol) { checkNotNull(); }
  ...
  const RDKix::RWMol &get() const {
    checkNotNull();
    return static_cast<RDKix::RWMol &>(*d_mol.get());
  }
  RDKix::RWMol &get() {
    checkNotNull();
    return static_cast<RDKix::RWMol &>(*d_mol.get());
  }
  const RDKix::ROMOL_SPTR &get_sptr() const { return d_mol; }
  RDKix::ROMOL_SPTR &get_sptr() { return d_mol; }

 private:
  void checkNotNull() const { CHECK_INVARIANT(d_mol, "d_mol cannot be null"); }
  RDKix::ROMOL_SPTR d_mol;
};

class JSMolList {
 public:
  JSMolList(const std::vector<RDKix::ROMOL_SPTR> &mols)
      : d_mols(mols), d_idx(0){};
  JSMolList() : d_idx(0){};
  JSMolBase *next();
  ...
  const std::vector<RDKix::ROMOL_SPTR> &mols() const { return d_mols; }
...
class JSLog {
 public:
  JSLog(RDKix::MinimalLib::LogHandle *logHandle);
  ~JSLog();
  std::string get_buffer() const;
  void clear_buffer() const;

 private:
  RDKix::MinimalLib::LogHandle *d_logHandle;
};

#ifdef RDK_BUILD_MINIMAL_LIB_RXN
class JSReaction {
 public:
  JSReaction() : d_rxn(new RDKix::ChemicalReaction()) {}
  JSReaction(RDKix::ChemicalReaction *rxn) : d_rxn(rxn) { assert(d_rxn); }
  [[deprecated(
